...
const features = [
  ...
  "packs.feat.video", "packs.feat.logo_maquette", "packs.feat.stand",
  "packs.feat.radio", "packs.feat.ceremony", "packs.feat.formation",
  "packs.feat.badge", "packs.feat.thanks",
];

const packAccess: Record<string, number> = { bronze: 3, silver: 6, gold: 9, diamond: 11 };
          ...
          {packs.map((pack) => {
            ...
            return (
              <motion.div
                ...
                className={`card-gradient rounded-xl border-2 ${pack.styles} p-6 flex flex-col ${pack.highlight ? "scale-[1.02]" : ""} cursor-pointer transition-transform group`}
              >
                  ...
                  {features.map((feat, i) => {
                    ...
                    return (
                      <li key={feat} className="flex items-start gap-2 text-xs font-body">
                        {included ? <Check className="w-4 h-4 text-primary shrink-0 mt-0.5" /> : <X className="w-4 h-4 text-muted-foreground/30 shrink-0 mt-0.5" />}
                        <span className={included ? "text-foreground" : "text-muted-foreground/40"}>{t(feat)}</span>
                      </li>
                    );
                  })}
                </ul>
